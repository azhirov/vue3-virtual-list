/** Based on https://github.com/vuetifyjs/vuetify/blob/master/packages/vuetify/src/composables/virtual.ts */
...
export function useVirtual <T> (props: VirtualProps, items: Ref<readonly T[]>, minItemHeight: MaybeRef<number> = 16) {
  ...
  watchEffect(() => {
    itemHeight.value = parseFloat(props.itemHeight?.toString() || '0')
  })
  ...
  const viewportHeight = computed(() => {
    return containerRef.value === document.documentElement
      ...
      : contentRect.value?.height || parseInt(props.height?.toString() || '0') || 0
  })
